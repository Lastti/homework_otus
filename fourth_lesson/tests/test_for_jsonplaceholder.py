...
from fourth_lesson.src.jsonplaceholder_api import posts_endpoint, main_schema, single_schema, comments_schema


def test_get_all_posts():
    response = requests.get(posts_endpoint)
    assert response.ok, f'Response status code: {response.status_code} is not successful'
    validate(instance=response.json(), schema=main_schema)
...
def test_get_any_post(id):
    ...
    assert response.ok, f'Response status code: {response.status_code} is not successful'
    validate(instance=response.json(), schema=single_schema)
